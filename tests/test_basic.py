...
class TestBasicScan:
    """Test basic scan_ducklake and read_ducklake functionality."""

    def test_scan_returns_lazyframe(self, ducklake_catalog):
        ...
        result = scan_ducklake(cat.metadata_path, "test")
    ...
    def test_basic_read(self, ducklake_catalog):
        ...
        expected = pl.DataFrame(
            {"a": pl.Series([1, 2], dtype=pl.Int32), "b": ["hello", "world"]}
        )
    ...
    def test_read_with_column_selection(self, ducklake_catalog):
        ...
        expected = pl.DataFrame(
            {"a": pl.Series([1], dtype=pl.Int32), "c": [3.14]}
        )
        assert_frame_equal(result, expected)
    ...
    def test_multiple_inserts(self, ducklake_catalog):
        ...
        assert result.sort("a")["a"].to_list() == [1, 2, 3]
    ...
    def test_create_table_as_select(self, ducklake_catalog):
        ...
        assert result["a"].to_list() == list(range(10))
        assert result["b"].to_list() == [i * 2 for i in range(10)]

    def test_lazy_filter(self, ducklake_catalog):
        cat = ducklake_catalog
        cat.execute(
            "CREATE TABLE ducklake.test AS SELECT i AS a FROM range(100) t(i)"
        )
        cat.close()

        result = scan_ducklake(cat.metadata_path, "test").filter(pl.col("a") > 95).collect()
        assert sorted(result["a"].to_list()) == [96, 97, 98, 99]

    def test_lazy_select(self, ducklake_catalog):
        cat = ducklake_catalog
        cat.execute("CREATE TABLE ducklake.test (a INTEGER, b VARCHAR, c DOUBLE)")
        cat.execute("INSERT INTO ducklake.test VALUES (1, 'hello', 3.14)")
        cat.close()

        result = scan_ducklake(cat.metadata_path, "test").select("a", "c").collect()
        assert result.columns == ["a", "c"]
        assert result.row(0) == (1, 3.14)
    ...
    def test_null_values(self, ducklake_catalog):
        ...
        assert result["a"].to_list() == [1, None, None]
