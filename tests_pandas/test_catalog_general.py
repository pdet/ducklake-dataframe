"""Catalog and general tests for ducklake-pandas."""
...
from ducklake_pandas import DuckLakeCatalog, read_ducklake
...
class TestNonDefaultSchema:
    ...
    def test_scan_from_custom_schema(self, ducklake_catalog):
        ...
        assert result["b"].tolist() == ["beta", "gamma"]
    ...
    def test_multiple_schemas(self, ducklake_catalog):
        ...
        assert result_s1["a"].tolist() == [1]
        assert result_s1["b"].tolist() == ["from_s1"]

        assert result_s2.shape == (1, 2)
        assert result_s2["a"].tolist() == [2]
        assert result_s2["b"].tolist() == ["from_s2"]
...
class TestQuotedIdentifiers:
    ...
    def test_column_with_special_chars(self, ducklake_catalog):
        ...
        assert result["my_col"].tolist() == [42]
        assert result["MixedCase"].tolist() == ["test_val"]
        assert result["another_one"].tolist() == [3.14]
...
class TestCatalogListWithSchemas:
    ...
    def test_list_schemas_includes_custom(self, ducklake_catalog):
        ...
        schema_names = result["schema_name"].tolist()
...
class TestMultipleTablesRead:
    ...
    def test_read_two_tables_different_schemas(self, ducklake_catalog):
        ...
        assert orders["order_id"].tolist() == [1, 2]
        assert orders["amount"].tolist() == [99.99, 149.50]

        customers = customers.sort_values("customer_id").reset_index(drop=True)
        assert customers.shape == (2, 2)
        assert customers["customer_id"].tolist() == [10, 20]
        assert customers["name"].tolist() == ["Alice", "Bob"]
...
class TestEdgeCaseReads:
    ...
    def test_wide_table(self, ducklake_catalog):
        ...
        assert result["c0"].tolist() == [0]
        assert result["c25"].tolist() == [25]
        assert result["c49"].tolist() == [49]
    ...
    def test_many_small_inserts(self, ducklake_catalog):
        ...
        assert result["b"].tolist() == [f"row_{i}" for i in range(20)]
